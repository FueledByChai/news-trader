"""
bot_oil.py  —  Session-Based Contrarian Trading Bot (Brent Crude)
─────────────────────────────────────────────────────────────────
Strategy:
  At the open of each session (Asia 00:00, London 08:00, NY 13:00 UTC):
    1. Fetch oil + geopolitical news published since the previous session open
    2. Ask Claude: what is the net impact on oil prices?
    3. Go CONTRARIAN — bullish oil news → SHORT, bearish oil news → LONG
...
Run:
  python bot_oil.py               # uses config_oil.yaml
  python bot_oil.py --dry-run     # force paper mode
  python bot_oil.py --once        # evaluate once and exit
  python bot_oil.py --config x.yaml
"""
...
from news_fetcher_oil import fetch_session_news
from sentiment_engine_oil import score_session
...
def setup_logging(cfg: dict):
    ...
    log_file = cfg.get("logging", {}).get("log_file", "bot_oil.log")
...
def evaluate_and_trade(trader: ParadexTrader, cfg: dict,
    ...
    """
    Fetch oil + geopolitical news since session_open, score with Claude,
    determine contrarian direction, and open/flip/hold accordingly.
    """
    ...
    # 1. Fetch oil + geopolitical news for this session window
    ...
    log.info(f"  STATUS  {market}=${price:,.2f}  balance=${balance:,.2f}  "
...
def main():
    parser = argparse.ArgumentParser(description="Brent Crude Contrarian Bot")
    parser.add_argument("--config",  default="config_oil.yaml")
    ...
    if cfg["api_keys"]["anthropic"].startswith("YOUR_") or cfg["api_keys"]["anthropic"] == "KEY_HERE":
        log.error("Set your Anthropic API key in config_oil.yaml or .env")
        sys.exit(1)

    paper  = cfg["trading"]["paper_trading"]
    market = cfg["paradex"]["market"]
    log.info("Brent Crude Contrarian Bot")
    ...
    log.info(f"   Always in market — bullish oil news -> SHORT, bearish -> LONG")
    log.info(f"   News: oil RSS + geopolitical RSS + Google News")
