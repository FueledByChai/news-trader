...
dashboard.py
─────────────
Live terminal dashboard for monitoring the sentiment bot.
Reads signals.csv and trades.csv — works while bot.py is running.

Run:
  python dashboard.py
  python dashboard.py --refresh 10   # refresh every 10 seconds
"""
...
def score_bar(score: float, width: int = 30) -> str:
    ...
    if 0 <= pos < width:
        bar[pos] = "●"
    bar[mid] = bar[mid] if bar[mid] == "●" else "│"
    bar_str = "".join(bar)
...
def render_dashboard(signal_log: str, trade_log: str):
    ...
    print(color("━" * 65, "34"))
    print(color(f"  🤖 BTC SENTIMENT BOT DASHBOARD  {now}", "1;34"))
    print(color("━" * 65, "34"))
    ...
    print(color(f"  {'Time':19}  {'Score':>7}  {'Conf':>5}  {'Strength':10}  Action", "2"))
    for sig in reversed(signals[-8:]):
        ts = sig.get("timestamp", "")[:19]
        sc = float(sig.get("composite_score", 0))
        cf = float(sig.get("confidence", 0))
        st = sig.get("signal_strength", "")
        ac = sig.get("action", "")
        sc_colored = color(f"{sc:+.3f}", "32" if sc > 0.3 else "31" if sc < -0.3 else "33")
        ac_colored = color(f"{ac:12}", "32" if "LONG" in ac else "31" if "SHORT" in ac else "33")
        print(f"  {ts}  {sc_colored}  {cf:5.0%}  {st:10}  {ac_colored}")
...
def main():
    parser = argparse.ArgumentParser(description="BTC Sentiment Bot Dashboard")
    parser.add_argument("--signal-log", default="signals.csv")
    parser.add_argument("--trade-log", default="trades.csv")
    parser.add_argument("--refresh", type=int, default=15, help="Refresh interval in seconds")
    args = parser.parse_args()

    print("Starting dashboard... (waiting for log files)")
    while True:
        try:
            clear()
